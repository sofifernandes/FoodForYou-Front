...
@Component({
  selector: 'app-delete-interesse',
  templateUrl: './delete-interesse.component.html',
  styleUrls: ['./delete-interesse.component.css']
})
export class DeleteInteresseComponent implements OnInit {
  ...
  btnSim() {
    if (this.interesse.postagem.length != 0) {
      ...
      this.interesseService.deleteInteresse(this.interesse.id).subscribe(() => {
        this.router.navigate(['/cadastro-interesse'])
        this.alert.showAlertSuccess('Interesse apagado com sucesso!')
      })
    }
